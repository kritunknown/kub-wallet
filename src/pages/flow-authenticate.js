...
const AuthForm = ({config = {}, sessionId, onAuth = () => {}}) => {
  ...
  const authenticate = async e => {
    e.preventDefault()
    const {errors, data} = await api.authenticate({phone, passw})
  ...
  const requestOtp = async e => {
    e.preventDefault()
    const {errors, data} = await api.requestOtp({phone})

    console.log("%capi.requestOtp", "color:#0066ff;", {errors, phone, data})
    if (errors) setErrors(errors)
    
    console.log("%capi.requestOtp token", "color:#0066ff;", data.requestOtp.token)
    setToken(data.requestOtp.token)
    if(data.requestOtp.step==0){
      setStep('verify')
    }else{
      setStep('main')
    }
  ...
  const authenticateOtp = async e => {
    e.preventDefault()
    console.log("%capi.authenticateOtp", "color:#0066ff;", {phone, otp, token})
    const {errors, data} = await api.authenticateOtp({phone, otp, token})
    //if (DEBUG)
    console.log("%capi.authenticateOtp", "color:#0066ff;", {errors, data})
    // if (errors){
    //   setStep('main')
    //   setErrors(errors)
    // }else{
    //   
    // }
    if(data.authenticateOtp.step==2){
      setToken(data.authenticateOtp.token)
      setStep('update')
    }else{
      onAuth(data.authenticateOtp.sessionId)
    }
  }

  const authenticateUpdate = async e => {
    e.preventDefault()
    const {errors, data} = await api.authenticateUpdate({phone, passw, token})

    console.log("%capi.authenticateUpdate", "color:#0066ff;", {errors, data})

    if (errors) setErrors(errors)
    onAuth(data.authenticateUpdate.sessionId)
  }
  ...
  const render_update = () => {
    return html`
    <form onSubmit=${authenticateUpdate}>
      ...
      <div>
        <button onClick=${authenticateUpdate}>
          Setup password
        </button>
      </div>
    </form>
  ...
  const render_otp = () => {
    ...
    <form onSubmit=${authenticateOtp}>
      ...
      <div>
        <button onClick=${authenticateOtp}>
          Verify
        </button>
      </div>
    </form>
  ...
  const render_phone = () => {
    ...
    <form onSubmit=${requestOtp}>
      ...
      <a onClick=${loginWithPass}>Login with pass</a>
    </form>
  `
  }
  ...
  const render_pass = () => {
    ...
    <form onSubmit=${authenticate}>
      ...
      <a onClick=${loginWithPhone}>Login with phone</a>
    </form>
  `
  }


  return type=='phone'? (step=='main'? render_phone() : (step=='verify'? render_otp() : render_update()) ) : render_pass();
}
